...
@Serializable data class ResponseMessage(val correlationId: String, val payload: String)
...
@Serializable
data class UserInfo(val id: String, val email: String, val name: String, val createdAt: String)
            ...
            when (requestMsg.type) {
                "register" -> {
                    ...
                    val authResponse: AuthResponse = transaction {
                        val existingUser =
                                Users.select { Users.email eq registerRequest.email }.singleOrNull()
                ...
                "login" -> {
                    ...
                    val authResponse: AuthResponse = transaction {
                        val userRow =
                                Users.select { Users.email eq loginRequest.email }.singleOrNull()
                        if (userRow != null &&
                                        BCrypt.checkpw(
                ...
                "update-profile-metrics" -> {
                    ...
                    val profile = transaction {
                        ...
                        if (existing != null) {
                            ...
                            if (upd.height != null &&
                                            (prevHeight == null || upd.height != prevHeight)
                            ) {
                            ...
                            if (upd.weight != null &&
                                            (prevWeight == null || upd.weight != prevWeight)
                            ) {
                                UserMetricsHistory.insert { stmt ->
                                    stmt[UserMetricsHistory.userId] = userId
                                    stmt[field] = "weight"
                    ...
                    sendResponse<UserProfile>(requestMsg.correlationId, profile, keydb)
                }
                "get-metrics-history" -> {
                    ...
                    val result = transaction {
                        val baseQuery =
                                UserMetricsHistory.select { UserMetricsHistory.userId eq userId }
                        ...
                        val filtered =
                                baseQuery.let { q0 ->
                                    ...
                                    params.from?.let {
                                        ...
                                        q =
                                                q.andWhere {
                                                    UserMetricsHistory.changedAt greaterEq fromDt
                                                }
                                    }
                                    params.to?.let {
